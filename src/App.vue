...
const getCompletion = async () => {
  ...
  result.value = (await response.json()).message.content;
...
<template>
  <div
    class="flex max-w-7xl mx-auto flex-col items-center justify-center min-h-screen"
  >
    <TheHeader />
    <main
      class="flex flex-1 w-full flex-col items-center justify-center text-center px-4 mt-4"
    >
      <div class="flex flex-col gap-4">
        <img :src="Icon" class="w-64 h-64 mx-auto" />
        <p
          class="mx-auto font-sans text-4xl font-extrabold max-w-xl text-center"
        >
          Find your next delicious meal with ease using our
          <span
        ...
        <div class="flex items-center my-4">
          <span
            class="w-8 h-8 rounded-full font-medium bg-black text-white inline-flex items-center justify-center mr-2"
            >1</span
          >
        ...
        <div v-if="places.length" class="text-center max-w-[800px]">
          ...
          <textarea
            rows="4"
            class="w-full rounded-md shadow focus:border-black focus:ring-black p-4"
            placeholder="e.g. I'm looking for a place to eat seafood with my family."
            v-model="context"
          ></textarea>
          <button
            @click="getCompletion"
          ...
          >
            {{ isLoading ? "Loading..." : "Generate recommendation" }}
          </button>
          <template v-if="result">
            <div class="mt-8">
              <h2 class="text-3xl font-bold text-slate-900 mx-auto">
                Your generated recommendation
              </h2>
